FILESEXTRAPATHS:prepend := "${THISDIR}/musl:${THISDIR}/files:"
...
# so that all of the files that end up in /usr/lib and /usr/etc are matched and # driven by the one same variable - which is not the case currently.
