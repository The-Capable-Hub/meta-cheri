...
PACKAGECONFIG:remove:pn-libcxx:cheri = "unwind-shared"

# Cheri __libuwind_config.h breaks on cross-unwinding _LIBUNWIND_CAPREG_START undefined but needed for mips TODO: Fix this
EXTRA_OECMAKE:remove:cheri = "-DLIBUNWIND_ENABLE_CROSS_UNWINDING=ON"

# baremetal build does not have shared libraries
EXTRA_OECMAKE:remove:cheri = "-DLIBCXXABI_ENABLE_SHARED=ON"
EXTRA_OECMAKE:append:cheri = " -DLIBCXXABI_ENABLE_SHARED=OFF"

# baremetal build needs extra cmake config
EXTRA_OECMAKE:append:cheri = "\
...
# so remove them
CXXFLAGS:remove:cheri = "-stdlib=libstdc++"
LDFLAGS:remove:cheri = "-unwindlib=libgcc -stdlib=libstdc++"
